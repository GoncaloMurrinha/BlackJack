...
class BlackjackMesaGUI:
    def __init__(self, root):
        ...
        self.root.state('zoomed')  # fullscreen maximizado
        # self.root.attributes('-fullscreen', True)  # alternativo: fullscreen total
        ...
        # Estado do jogo
        self.deck = []
        self.players = [{'hand': [], 'balance': self.start_balance}]  # Sempre começa com 1 jogador
        self.dealer_hand = []
        ...
        # Estilo bonito para o botão + (deve ser antes do draw_table)
        style = ttk.Style()
        style.configure("AddPlayer.TButton",
                        ...
                        relief="flat",
                        padding=0)
        style.map("AddPlayer.TButton",
                  background=[('active', '#51e67d'), ('!active', '#2ecc40')],
                  foreground=[('active', 'white')])
        self.add_player_style = "AddPlayer.TButton"
        ...
        # Controles e status
        self.status = tk.Label(self.root, text="Adicione jogadores com + e clique Iniciar", bg="#064820", fg="#f6e27f", font=("Segoe UI", 16, "bold"))
        self.status.pack(pady=10)

        self.controls_frame = tk.Frame(self.root, bg="#064820")
        self.controls_frame.pack(pady=10)

        self.btn_start = ttk.Button(self.controls_frame, text="Iniciar Jogo", command=self.setup_game)
        ...
        # Não precisa mais de labels de saldo fora do canvas
        self.add_player_buttons = [None for _ in range(self.max_players)]
        self.draw_table()
        self.update_ui()
        self.btn_start.config(state=tk.NORMAL)
        # Corrigir posição dos botões + após layout
        self.root.after(100, self.update_add_player_buttons)
        self.canvas.bind('<Configure>', lambda e: self.update_add_player_buttons())

    def draw_table(self):
        w, h = 1100, 600
        self.canvas.delete("all")
        self.canvas.create_arc(50, 50, w-50, h*2, start=0, extent=180, fill="#0a2d11", outline="#f6e27f", width=6)
        self.canvas.create_text(w//2, 300, text="DEALER", fill="#f6e27f", font=("Segoe UI", 18, "bold"))
        self.player_positions = self.get_player_positions()
        for i, (x, y) in enumerate(self.player_positions):
            ...
            self.canvas.create_text(x, y+60, text=f"Jogador {i+1}", fill="#f6e27f", font=("Segoe UI", 14, "bold"))
    ...
    def start_round(self):
        if len(self.players) == 0:
            self.status.config(text="Adicione pelo menos 1 jogador!")
            return
    ...
    def update_ui(self, reveal_dealer=True):
        ...
        for i, (x, y) in enumerate(self.player_positions):
            if i < len(self.players):
                ...
                # Pontos por baixo das cartas, acima do saldo
                pts = self.score(p['hand'])
                self.canvas.create_text(x, y+80, text=f"Pontos: {pts}", fill="white", font=("Segoe UI", 13, "bold"), tags="card")
                # Saldo debaixo do círculo, mais afastado
                self.canvas.create_text(x, y+100, text=f"Saldo: ${p['balance']}", fill="#f6e27f", font=("Segoe UI", 13, "bold"), tags="card")
            else:
                pass
        self.update_add_player_buttons()
    ...
    def update_add_player_buttons(self):
        # Cria ou atualiza os botões + nas bolas vazias
        for i in range(self.max_players):
            if i < len(self.players):
                if self.add_player_buttons[i] is not None:
                    self.add_player_buttons[i].place_forget()
            else:
                x, y = self.player_positions[i]
                if self.add_player_buttons[i] is None:
                    btn = ttk.Button(self.root, text="+", style=self.add_player_style, command=lambda idx=i: self.add_player(idx))
                    btn.bind("<Enter>", lambda e, b=btn: b.configure(cursor="hand2"))
                    btn.bind("<Leave>", lambda e, b=btn: b.configure(cursor=""))
                    self.add_player_buttons[i] = btn
                else:
                    btn = self.add_player_buttons[i]
                # Centralizar perfeitamente
                btn.place(x=self.canvas.winfo_rootx() - self.root.winfo_rootx() + x - 28,
                          y=self.canvas.winfo_rooty() - self.root.winfo_rooty() + y - 28,
                          width=56, height=56)
                btn.lift()

    def add_player(self, idx):
        if len(self.players) < self.max_players:
            self.players.append({'hand': [], 'balance': self.start_balance})
            self.update_ui()
        if len(self.players) > 0:
            self.btn_start.config(state=tk.NORMAL)
